...
class _ProductItemScreenState extends State<ProductItemScreen> {
  ...
  late List Colorss = [];
  late List Sizes = [];
  _ProductItemScreenState(this.name, this.price, this.discount, this.after,
 ...
 late String coller;
  ...
  Widget getItems(List Colorss) {
    FirebaseAuth auth= FirebaseAuth.instance;
    String uid=auth.currentUser!.uid.toString();
    CollectionReference ref = FirebaseFirestore.instance.collection(uid).doc("Colors").collection("Colors");
                    ...
                    child: GestureDetector(
                      ...
                      child: InkWell(
                        onTap: (){
                          ...
                          ref.doc(document['color']) // <-- Doc ID where data should be updated.
                              .update(
                              {
                                'isSelected':iscolor,
                              });
                          // String collor=document['colors'];
                         // print(Colorss[index]);
                          get(document['color']);
                        },
                        child: Container(
                            ...
                            decoration: BoxDecoration(
                              border: Border.all(color: Colors.black12),
                              color: document['isSelected']?Colors.red:Colors.white,

                            ),
                            child: Text(
                              document["color"],style: TextStyle(color:document['isSelected']?Colors.white:Colors.black, ),
                            )),
                      ),
                    ),
                  );
  ...
  Widget getSizes(List Sizes) {
    FirebaseAuth auth= FirebaseAuth.instance;
    String uid=auth.currentUser!.uid.toString();
    CollectionReference ref = FirebaseFirestore.instance.collection(uid).doc("Sizes").collection("Sizes");
    return StreamBuilder(
        stream: ref.snapshots(),
        builder: (BuildContext context,
            AsyncSnapshot<QuerySnapshot> snapshot) {
                    ...
                    child: GestureDetector(
                      ...
                      child: InkWell(
                        onTap: (){
                          ...
                          ref.doc(document['size']) // <-- Doc ID where data should be updated.
                              .update(
                              {
                                'isSelected':issize,
                              });
                        ...
                        child: Container(
                            ...
                            child: Text(
                              document["size"],style: TextStyle(color:document['isSelected']?Colors.white:Colors.black, ),
                            )),
                      ),
                    ),
                  );
